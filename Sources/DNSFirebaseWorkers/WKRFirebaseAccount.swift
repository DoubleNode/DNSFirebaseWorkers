    ...
    override open func intDoLoadLinkRequests(for user: DAOUser,
        ...
        self.processRequestData(callData, dataRequest, with: resultBlock,
            ...
            do {
                ...
                let linkRequests = response.linkRequests.map { linkRequestData in
                    let linkRequest = Self.createLinkRequest()
                    linkRequest.account = linkRequestData.account
                    linkRequest.user = linkRequestData.user
                    linkRequest.approved = linkRequestData.approved ?? Date()
                    linkRequest.approvedBy = linkRequestData.approvedBy
                    linkRequest.requested = linkRequestData.requested
                    return linkRequest
                }
                block?(.success(linkRequests))
                return .success
            } catch {
                DNSCore.reportError(error)
                return .failure(error)
